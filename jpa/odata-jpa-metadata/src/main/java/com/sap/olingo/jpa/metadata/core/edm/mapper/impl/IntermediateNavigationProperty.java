 ...
 * entity type and allows to navigate to it. IntermediateNavigationProperty
 * represents a navigation within on service, that is source and target are
 * described by the same service document. <a href=
 ...
 *
 */
final class IntermediateNavigationProperty extends IntermediateModelElement
		...
		if (edmNaviProperty == null) {
			...
			if (jpaAttribute.getJavaMember() instanceof AnnotatedElement) {
				...
				case ONE_TO_MANY:
					...
					isSourceOne = true;
					edmNaviProperty
							.setOnDelete(edmOnDelete != null ? edmOnDelete : setJPAOnDelete(cardinalityOtM.cascade()));
					break;
				case ONE_TO_ONE:
					...
					isSourceOne = true;
					edmNaviProperty
							.setOnDelete(edmOnDelete != null ? edmOnDelete : setJPAOnDelete(cardinalityOtO.cascade()));
					break;
				case MANY_TO_ONE:
					...
					edmNaviProperty
							.setOnDelete(edmOnDelete != null ? edmOnDelete : setJPAOnDelete(cardinalityMtO.cascade()));
					break;
				case MANY_TO_MANY:
					final ManyToMany cardinalityMtM = annotatedElement.getAnnotation(ManyToMany.class);
					mappedBy = cardinalityMtM.mappedBy();
					edmNaviProperty
							.setOnDelete(edmOnDelete != null ? edmOnDelete : setJPAOnDelete(cardinalityMtM.cascade()));
					break;
				default:
					break;
				}
...
//      Determine referential constraint
	...
	private void buildJoinColumns(final String mappedBy, final boolean isSourceOne,
		...
		if (mappedBy != null && !mappedBy.isEmpty()) {
			// Get
			IntermediateNavigationProperty navigationProperty = (IntermediateNavigationProperty) targetType
					.getAssociation(mappedBy);
			if (navigationProperty != null && navigationProperty.getJoinTable()!=null) {
				joinTable = ((IntermediateJoinTable) navigationProperty.getJoinTable());
				//
				joinColumns.addAll(
						joinTable == null ? buildJoinColumnsMapped(mappedBy) : joinTable.buildInverseJoinColumns());
			}
			//
			joinTable = joinTable == null ? null : joinTable.asMapped(this);
		} else
			joinColumns.addAll(joinTable == null ? buildJoinColumnsFromAnnotations(isSourceOne, annotatedElement)
					: joinTable.buildJoinColumns());
	}
	...
	private List<IntermediateJoinColumn> buildJoinColumnsFromAnnotations(final boolean isSourceOne,
		...
		final JoinColumn[] columns = annotatedElement.getAnnotation(JoinColumns.class) != null
				? annotatedElement.getAnnotation(JoinColumns.class).value()
				: null;
	...
	private void determienReferentialConstraints(final AnnotatedElement annotatedElement)
	...
	private void fillMissingName(final boolean isSourceOne, final IntermediateJoinColumn intermediateColumn)
		...
		if (isSourceOne && (refColumnName == null || refColumnName.isEmpty()))
			intermediateColumn.setReferencedColumnName(
					((IntermediateSimpleProperty) ((IntermediateEntityType) sourceType).getKey().get(0))
							.getDBFieldName());
		else if (isSourceOne && (name == null || name.isEmpty()))
			intermediateColumn.setReferencedColumnName(
					((IntermediateSimpleProperty) ((IntermediateEntityType) targetType).getKey().get(0))
							.getDBFieldName());
