...
/**
 * <p>
 * For details about Schema metadata see: <a href=
...
//  edm:TypeDefinition
	...
	JPAFunction getFunction(final String externalName) {
		for (final Entry<String, IntermediateFunction> func : functionListInternalKey.entrySet()) {
			if (func.getValue().getExternalName().equals(externalName) && !func.getValue().ignore())
	...
	IntermediateStructuredType getStructuredType(final PluralAttribute<?, ?, ?> jpaAttribute) {
		IntermediateStructuredType type = complexTypeListInternalKey
				.get(IntNameBuilder.buildStructuredTypeName(jpaAttribute.getElementType().getJavaType()));
		if (type == null)
			type = entityTypeListInternalKey
					.get(IntNameBuilder.buildStructuredTypeName(jpaAttribute.getElementType().getJavaType()));
		return type;
	}
	...
	IntermediateStructuredType getStructuredType(final Attribute<?, ?> jpaAttribute) {
		IntermediateStructuredType type = complexTypeListInternalKey
				.get(IntNameBuilder.buildStructuredTypeName(jpaAttribute.getJavaType()));
		if (type == null)
		...
		if (reflections != null) {
			for (Class<?> enumeration : reflections.getTypesAnnotatedWith(EdmEnumeration.class)) {
				if (enumeration.isEnum()) {
					@SuppressWarnings("unchecked")
					final IntermediateEnumerationType e = new IntermediateEnumerationType(nameBuilder,
							(Class<T>) enumeration);
